...
class PngQuant(object):
    ...
    def __init__(self):
        ...
        #self.command_str = '{quant_file} --quality={min_quality}-{max_quality} -'
        ...
        self.ndeep = 100
        self.ndigits = 4
        self.tmp_file = os.path.join(tempfile.gettempdir(), 'quant.tmp.'+str(time.time())+'.png')
    ...
    def quant_compress(self, data=None, ndeep=None):
        ...
        while ndeep and (compressed_len < origin_len):
            ...
            try:
                ...
                #a = self.command_line.split(" ")
                #p = subprocess.Popen(a, stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, bufsize=-1, shell=True)
                #dat, er = p.communicate(input=origin_data)
                #compressed_data = dat
                #while p.returncode == None:
                #    dat, er = p.communicate()
                #    compressed_data += dat
